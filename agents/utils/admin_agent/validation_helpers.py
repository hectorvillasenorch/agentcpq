...
def validate_rule_update_request(name, field, value, session_context):
    response_message = ""

    if name is None:
        response_message += f"⚠️ Error: No rule name to update was detected in your request. Please specify the rule name.<br><br>"
        agent_response = f"Error: No rule name to update was detected in your request. Please specify the rule name."
    ...
    try:
        ...
        agent_response = f"Error: The rule with the name {name} does not exist."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None
    ...
    if field is None:
        ...
        agent_response = f"Error: No field to update was detected in your request. Please specify which attribute (e.g., rule type, target type, priority, error message, active, description or conditions) you want to modify."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None
    ...
    if field not in available_fields:
        ...
        agent_response = f"Error: '{field}' is not a recognized field. Allowed fields are: {', '.join(available_fields)}."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None

    if value is None:
        response_message += f"⚠️ Error: No value to update was detected in your request.<br><br>"
        agent_response = f"Error: No value to update was detected in your request."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None
    ...
    if field in string_fields and not isinstance(value, str):
        ...
        agent_response = f"Error: The field '{field}' must be updated with a string value."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None
    ...
    if field == "rule_type":
        ...
        if value not in allowed_rule_types:
            ...
            agent_response = f"Error: The field 'rule type' must be one of: {', '.join(allowed_rule_types)}."
            save_or_update_conversation_context(session_context, agent_response)
            return False, response_message, None
    ...
    if field == "target_type":
        ...
        if value not in allowed_target_types:
            ...
            agent_response = f"Error: The field 'target type' must be one of: {', '.join(allowed_target_types)}."
            save_or_update_conversation_context(session_context, agent_response)
            return False, response_message, None
    ...
    if field == "priority" and not isinstance(value, int):
        ...
        agent_response = f"Error: The field 'priority' must be an integer (e.g., 1, 5, 10)."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None
    ...
    if field == "active" and not isinstance(value, bool):
        ...
        agent_response = f"Error: The field 'active' must be a boolean value: use `true` to enable the rule, or `false` to disable it."
        save_or_update_conversation_context(session_context, agent_response)
        return False, response_message, None
    ...
    if field == "conditions":
        if not isinstance(value, dict):
            ...
            agent_response = f"Error: The field 'conditions' must be a JSON object with a 'logic' key and an 'items' list."
            save_or_update_conversation_context(session_context, agent_response)
            return False, response_message, None
        ...
        if "logic" not in value or "items" not in value:
            ...
            agent_response = f"Error: The 'conditions' object must include both 'logic' (e.g., 'AND') and 'items' (a list of condition objects)."
            save_or_update_conversation_context(session_context, agent_response)
            return False, response_message, None
        ...
        if not isinstance(value["logic"], str) or not isinstance(value["items"], list):
            ...
            agent_response = f"Error: In the 'conditions' object, 'logic' must be a string (e.g., 'AND'), and 'items' must be a list of conditions."
            save_or_update_conversation_context(session_context, agent_response)
            return False, response_message, None
